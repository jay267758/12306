...
function mover01(oDom, attr, startP, endP, step, direction, timeSpace) {
  ...
  var myTimer = setInterval(function () {
    // 一、数据处理
    // 1、运算
    value = value + direction * step; // 2、边界处理（合法性的处理）
    // if (value >= endP) {
...
function mover03(oDom, attr, endP, timeLong, fn) {
  // 起始值也可以获取到
  var startP = parseFloat(getStyle(oDom, attr));
  var value = startP; // 方向可以在内部进行计算

  var direction = startP < endP ? 1 : -1; // 步长和时间间隔合起来决定了速度（步长越长，越快，事件间隔越短，越快）
  // 已知 时长：timeLong
...
function sliderH(domOut, domIn, attr, endP, timeLong, fn) {
  ...
  var myTimer = setInterval(function () {
    ...
    domIn.style[attr] = value + diffLeft + "px"; // 说明定时器停止了，即：滑动停止了
